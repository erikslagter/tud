function initPage()
{initTabs();initDropDown();initNavIndexes();initSearch();clearFormFields({clearInputs:true,clearTextareas:true,passwordFieldText:true,addClassFocus:"focus",filterClass:"default"});}
function initSearch(){var radioName="group1";var changerName="changer";var _form=document.getElementById("search-form");if(_form===null){return;}
var _inp=_form.getElementsByTagName("input");var _search=false;var _val=false;var _changhval="keyword";for(var i=0;i<_inp.length;i++){if(_inp[i].type=="text"&&_inp[i].name=="fsearch"){_search=_inp[i];_val=_search.value;}}
if(_search){for(var i=0;i<_inp.length;i++){if(_inp[i].type=="radio"&&_inp[i].name==radioName){_inp[i].onclick=function(){if(this.className.indexOf(changerName)!=-1){_search.value=_changhval;}else{_search.value=_val;}}}}}}
...
{var nav=document.getElementById("nav");if(nav){var lis=nav.getElementsByTagName("li");for(var i=0;i<lis.length;i++){if(lis[i].getElementsByTagName("ul").length>0){lis[i].className+=" has-drop-down"
...
{var nav=document.getElementById("nav");if(nav){var lis=nav.getElementsByTagName("li");for(var i=0;i<lis.length;i++){lis[i].style.zIndex=lis.length-i;}}}
